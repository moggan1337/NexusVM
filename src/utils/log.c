/**
 * NexusVM Utility Functions Implementation
 */
...
{
    const unsigned long *p = addr + (offset / (sizeof(unsigned long) * 8));
    unsigned long result = offset & ~(sizeof(unsigned long) * 8 - 1);
    unsigned long tmp;
    ...
    while (size & ~(sizeof(unsigned long) * 8 - 1)) {
        tmp = *p++;
        if (tmp) {
            goto found_middle;
        }
...
found_middle:
    result += __builtin_ctzl(tmp);
found:
    return result < offset ? offset : result;
}
